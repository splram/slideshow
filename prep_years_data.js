const SunCalc = require("./views/suncalc")
d2r = Math.PI / 180;
r2d = 180 / Math.PI;
...
//      console.log("current month", d1, getCurrentMonth(d1))
   ...
   init : function() {
      ...
      for (var y = beginYear; y <= (d1.getFullYear() + beginYear); y++) {
        adhikaMasaFlag = false;
        var curMonths = [];
         var fileName = "";
         ...
         let s1 = getNextNewMoon(d1, false);
         if (monthCount == -1) {
            monthCount = s1.getMonth() + 1;
         }
         let s2 = new Date(s1);
         s2.setDate(s2.getDate() + 10);
         s2 = getNextNewMoon(s2)
         fileName = s1.getFullYear();
         curMonths.push({
            name : masa[0],
            start:s1,
            end:s2
         ...
         if (monthCount % 33 === 0) {
            adhikaMasaFlag = true;
            let astart = new Date(s2);
            astart.setDate(astart.getDate() + 1);
            astart = getNextNewMoon(astart);

            let aend = new Date(astart);
            aend.setDate(aend.getDate() + 1);
            aend = getNextNewMoon(aend);
            var adhikaMasa = {
               name : masa[0],
               start : astart,
               end : aend
            ...
            monthCount++
            curMonths.push(adhikaMasa);
            //console.log("1111 Extra month", y, adhikaMasa);
            s2 = new Date(aend);
         ...
         for (var i = 1; i < 12; i++) {
            ...
            if (monthCount % 33 === 0) {
               ...
               //console.log("22222 Extra month", y, adhikaMasa);
               s2 = new Date(aend);
               s2.setDate(s2.getDate() + 1)
               monthCount++
               curMonths.push(adhikaMasa);             
        ...
        console.log("processing ", fileName)
            ...
            let jsonContent = JSON.stringify(year);
            //console.log(jsonContent)
            fs.writeFileSync(`./data/${fileName}.json`, jsonContent, 'utf8', function (err) {
                if (err) {
   ...
   buildAdhikaMasa : function() {
    ...
    dt.setMonth(02);
    dt.setDate(1);
    //console.log("next new moon", getNextNewMoon(dt, true))
    var s1 = null;
    var s2 = null;
    for (var i = 0; i < masa_en.length; i++) {
        if (s1 === null) {
            s1 = getNextNewMoon(dt,false);
        } else {
            s1 = panchang.months[panchang.months.length - 1].end;
        }
        s1.setDate(s1.getDate() + 1);
        s2 = new Date(s1);
        s2.setDate(s2.getDate() + 1);
        s2 = getNextNewMoon(s2, false);
        panchang.months.push({
            name : masa[i],
            start : s1,
            end : s2
        })
        ...
        console.log(panchang.months[panchang.months.length - 1]);
    }
   }
