...
class CalendarYvv{
	...
	createDayTag(){
		...
		diasOrd.fechCort.forEach(function(e){
			var div = $("<div class='d-flex border flex-fill w-100 justify-content-center p-2 panchang-margin-right'>").html(e);
			cont.append(div);
		});
		return cont;
	}
	createDaysMont(){
		...
		var emptyTag = "<div class='d-flex border flex-fill w-100 justify-content-center pt-3 pb-3 btn panchang-margin-right panchang-font-size' style='color:transparent'>";
		...
		for(var i=0;i<this.cantDias;i++){
			var _m = this.mesSeleccionado < 10 ? '0' + this.mesSeleccionado : this.mesSeleccionado;
			var fechNow = this.anioSeleccionado+"-"+_m+"-"+(i+1);
			var div = $("<div class='d-flex border flex-fill w-100 justify-content-center pt-3 pb-3 btn cell-height panchang-margin-right panchang-font-size "+this.btnD+"' data-date='"+fechNow+"'>").html(i+1);
			var clas_e = this;
			var _ind = (this.cantDias+posMes)%7;

			//dia seleccionado
			if(this.diaSeleccionado==fechNow){
				div = $("<div class='current-date-selected d-flex border flex-fill w-100 justify-content-center pt-3 pb-3 btn cell-height panchang-margin-right panchang-font-size "+this.btnD+"' data-date='"+fechNow+"'>").html(i+1);
			}
			//dias resaltados o importantes
			if(this.diasResal.indexOf(i+1)!=-1){
				div = $("<div class='d-flex border flex-fill w-100 justify-content-center pt-3 pb-3 btn cell-height panchang-margin-right  panchang-font-size "+this.btnD+"' data-date='"+fechNow+"' style='background: "+this.colorResal+"; color: "+this.textResalt+"; font-weight: bold;'>").html(i+1);
			}
